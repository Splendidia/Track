...
import sys
import argparse
sys.path.insert(0, '/home/dy/下载/OrientedFormer')

from tools.track_test.byte_tracker import Byte_tracker
from shapely.geometry import Polygon

# ========== 辅助函数（与 evaluate_tracking.py 相同） ==========
...
def run_tracker(video_folder, tracker, use_gt=False):
    ...
    for frame_id in frame_ids:
        ...
        #print(f"Reading image: {img_path}, exists={os.path.exists(img_path)}")
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run ablation experiments on selected videos.')
    parser.add_argument('--base_dir', type=str, default='/home/dy/NewDisk/pigtrackvideo',
                        help='Base directory containing video subfolders')
    parser.add_argument('--video_ids', type=int, nargs='+', required=True,
                        help='List of video IDs to process (e.g., 6 7 8 9 10)')
    ...
    parser.add_argument('--n_sig_components', type=int, default=5,
                        help='Number of signature components')
    parser.add_argument('--exp_name', type=str, default='selected_experiment',
...
def main():
    ...
    print(f"\n=== Running experiment: {args.exp_name} on videos {args.video_ids} ===")
