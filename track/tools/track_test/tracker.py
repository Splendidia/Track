...
from tools.track_test.fqa_gate import FQAGate  # 🚀 引入我们刚刚抽离出来的模块
...
class Tracker:
    ...
    def __init__(self, max_iou_distance=0.85, max_age=30, n_init=3,
                 ...
                 use_oamm=False, use_fac=False, use_rifm=False, n_sig_components=5, use_bev=False):
    ...
    def _initiate_track(self, rbox, conf, class_id, img_gray=None):
        track = Track(rbox, self._next_id, conf, class_id, self.n_init, self.max_age,
                      ...
                      use_oamm=self.use_oamm, use_rifm=self.use_rifm, n_sig_components=self.n_sig_components)
        self.tracks.append(track)
        self._next_id += 1

        # 🚀 使用模块化的 FQA 门控进行初始化检查
    ...
    def update(self, det, det_second, curr_gray=None, frame_id=None):
        ...
        #     'use_signature': self.use_signature, 'use_motion_memory': self.use_motion_memory,
        #     'use_oamm': self.use_oamm, 'use_rifm': self.use_rifm, 'rifm_extractor': getattr(self, 'rifm_extractor', None),
        #     'frame_id': frame_id,
    ...
    def get_active_tracks(self):
        return [t for t in self.tracks if t.is_confirmed() and t.time_since_update <= self.max_age]